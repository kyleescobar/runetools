...
import dev.runetools.asm.analysis.cfg.BlockHandler;
import dev.runetools.asm.analysis.exception.ResolvableExceptionFactory;
import dev.runetools.asm.analysis.exception.SimFailedException;
import dev.runetools.asm.analysis.exception.TypeMismatchKind;
import dev.runetools.asm.analysis.util.CollectUtils;
import dev.runetools.asm.analysis.value.*;
import dev.runetools.asm.analysis.value.simulated.AbstractSimulatedValue;
import dev.runetools.asm.analysis.value.simulated.ReflectionSimulatedValue;
import dev.runetools.asm.analysis.value.simulated.StringSimulatedValue;
import dev.runetools.asm.analysis.util.FlowUtil;
import dev.runetools.asm.analysis.value.*;
...
import static dev.runetools.asm.analysis.util.TypeUtil.*;
...
public class SimInterpreter extends Interpreter<AbstractValue> {
	...
	public AbstractValue copyOperation(AbstractInsnNode insn, AbstractValue value) throws AnalyzerException {
		...
		if(load && insnType != value.getType())
			return newValue(CollectUtils.add(value.getInsns(), insn), insnType);
		...
		switch(insn.getOpcode()) {
			case INEG:
				if (isValueUnknown(value))
					return newValue(CollectUtils.add(value.getInsns(), insn), Type.INT_TYPE);
				return PrimitiveValue.ofInt(CollectUtils.add(value.getInsns(), insn), -toInt(value));
			case IINC:
			...
			case FNEG:
				if (isValueUnknown(value))
					return newValue(CollectUtils.add(value.getInsns(), insn), Type.FLOAT_TYPE);
				return PrimitiveValue.ofFloat(CollectUtils.add(value.getInsns(), insn), -toFloat(value));
			case I2F:
			case L2F:
			case D2F:
				if (isValueUnknown(value))
			...
			case LNEG:
				if (isValueUnknown(value))
					return newValue(CollectUtils.add(value.getInsns(), insn), Type.LONG_TYPE);
				return PrimitiveValue.ofLong(CollectUtils.add(value.getInsns(), insn), -toLong(value));
			case I2L:
			case F2L:
			case D2L:
				if (isValueUnknown(value))
			...
			case DNEG:
				if (isValueUnknown(value))
					return newValue(CollectUtils.add(value.getInsns(), insn), Type.DOUBLE_TYPE);
				return PrimitiveValue.ofDouble(CollectUtils.add(value.getInsns(), insn), -toDouble(value));
			case I2D:
			case L2D:
			case F2D:
				if (isValueUnknown(value))
			...
			case GETFIELD: {
				...
				Type type = Type.getType(fin.desc);
				return newValue(CollectUtils.add(value.getInsns(), insn), type);
			}
			case NEWARRAY:
				switch(((IntInsnNode) insn).operand) {
					case T_BOOLEAN:
						return newValue(CollectUtils.add(value.getInsns(), insn), BOOLEAN_ARRAY_TYPE);
					case T_CHAR:
						return newValue(CollectUtils.add(value.getInsns(), insn), CHAR_ARRAY_TYPE);
					case T_BYTE:
						return newValue(CollectUtils.add(value.getInsns(), insn), BYTE_ARRAY_TYPE);
					case T_SHORT:
						return newValue(CollectUtils.add(value.getInsns(), insn), SHORT_ARRAY_TYPE);
					case T_INT:
						return newValue(CollectUtils.add(value.getInsns(), insn), INT_ARRAY_TYPE);
					case T_FLOAT:
						return newValue(CollectUtils.add(value.getInsns(), insn), FLOAT_ARRAY_TYPE);
					case T_DOUBLE:
						return newValue(CollectUtils.add(value.getInsns(), insn), DOUBLE_ARRAY_TYPE);
					case T_LONG:
						return newValue(CollectUtils.add(value.getInsns(), insn), LONG_ARRAY_TYPE);
					default:
						break;
				}
				throw new AnalyzerException(insn, "Invalid array type specified in instruction");
			case ANEWARRAY:
				return newValue(CollectUtils.add(value.getInsns(), insn), Type.getType("[" + Type.getObjectType(((TypeInsnNode) insn).desc)));
			case ARRAYLENGTH:
			...
			case CHECKCAST:
				...
				return newValue(CollectUtils.add(value.getInsns(), insn), Type.getObjectType(((TypeInsnNode) insn).desc));
			case INSTANCEOF:
				return newValue(CollectUtils.add(value.getInsns(), insn), Type.INT_TYPE);
			case MONITORENTER:
			case MONITOREXIT:
	...
	public AbstractValue binaryOperation(AbstractInsnNode insn, AbstractValue value1, AbstractValue value2)  {
		...
		switch(insn.getOpcode()) {
			case FALOAD:
				return newValue(CollectUtils.combineAdd(value1.getInsns(), value2.getInsns(), insn), Type.FLOAT_TYPE);
			case LALOAD:
				return newValue(CollectUtils.combineAdd(value1.getInsns(), value2.getInsns(), insn), Type.LONG_TYPE);
			case DALOAD:
				return newValue(CollectUtils.combineAdd(value1.getInsns(), value2.getInsns(), insn), Type.DOUBLE_TYPE);
			case AALOAD:
				if (value1.getType() == null)
					return newValue(CollectUtils.combineAdd(value1.getInsns(), value2.getInsns(), insn), OBJECT_TYPE);
				else
					return newValue(CollectUtils.combineAdd(value1.getInsns(), value2.getInsns(), insn),
							Type.getType(value1.getType().getDescriptor().substring(1)));
			case IALOAD:
			case BALOAD:
			case CALOAD:
			case SALOAD:
				return newValue(CollectUtils.combineAdd(value1.getInsns(), value2.getInsns(), insn), Type.INT_TYPE);
			case IF_ICMPEQ:
			case IF_ICMPNE:
			case IF_ICMPLT:
			case IF_ICMPGE:
	...
	public AbstractValue naryOperation(AbstractInsnNode insn, List<? extends AbstractValue> values) throws AnalyzerException {
		...
		if (opcode == MULTIANEWARRAY) {
			...
			return newValue(CollectUtils.add(argContributingInsns, insn),
		...
		if (opcode == INVOKEDYNAMIC) {
			...
			return newValue(CollectUtils.add(argContributingInsns, insn), retType);
			...
			try {
				...
				if (value != null) {
					value.addContributing(CollectUtils.disjoint(argContributingInsns, value.getInsns()));
					return value;
				}
	...
	public AbstractValue merge(AbstractValue value1, AbstractValue value2) {
		...
		List<AbstractInsnNode> merged =
				CollectUtils.distinct(CollectUtils.combine(value1.getInsns(), value2.getInsns()));
